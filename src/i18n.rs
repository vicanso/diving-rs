...
pub fn tr(lang: Lang, key: &str) -> &'static str {
    match (lang, key) {
        ...
        (Lang::En, "rec.runtimecompat.glibctitle") => "Binary needs newer glibc than base image",
        (Lang::Zh, "rec.runtimecompat.glibctitle") => "二进制要求的 glibc 版本高于基础镜像",
        (Lang::En, "rec.runtimecompat.musltitle") => "glibc binary in musl-based image",
        (Lang::Zh, "rec.runtimecompat.musltitle") => "glibc 二进制运行在 musl 镜像中",
        (Lang::En, "rec.runtimecompat.glibcmusltitle") => "musl binary in glibc-based image",
        (Lang::Zh, "rec.runtimecompat.glibcmusltitle") => "musl 二进制运行在 glibc 镜像中",
        ...
        (Lang::En, "rec.runtimecompat.glibcdetail") => "Entrypoint `{0}` requires GLIBC_{1} but the detected base image only provides glibc {2}. The container will fail at startup with `version GLIBC_{1} not found`.",
        (Lang::Zh, "rec.runtimecompat.glibcdetail") => "启动程序 `{0}` 要求 GLIBC_{1}，但检测到基础镜像只提供 glibc {2}。容器启动会报 `version GLIBC_{1} not found` 而立即失败。",
        (Lang::En, "rec.runtimecompat.musldetail") => "Entrypoint `{0}` is dynamically linked against glibc (GLIBC_{1}) but the base image ships musl (Alpine). The binary cannot execute at all — `not found` or segfault on launch.",
        (Lang::Zh, "rec.runtimecompat.musldetail") => "启动程序 `{0}` 动态链接到 glibc（GLIBC_{1}），但基础镜像使用 musl（Alpine）。该二进制根本无法执行——启动时会报 `not found` 或直接段错误。",
        (Lang::En, "rec.runtimecompat.glibcmusldetail") => "Entrypoint `{0}` is dynamically linked against musl but the base image is glibc-based. This usually works via the dynamic loader but is unusual — verify the binary actually loads.",
        (Lang::Zh, "rec.runtimecompat.glibcmusldetail") => "启动程序 `{0}` 动态链接到 musl，但基础镜像是 glibc 系。通常借助动态链接器仍可工作，但不常见——请确认该二进制确实能加载。",
        ...
        (Lang::En, "rec.runtimecompat.glibchint") => "Rebuild on a base image whose glibc is ≥ the required version (e.g. `debian:trixie-slim` for glibc 2.41, `debian:bookworm-slim` for 2.36), or build a static binary (`-C target-feature=+crt-static` for Rust, `CGO_ENABLED=0` for Go).",
        (Lang::Zh, "rec.runtimecompat.glibchint") => "在 glibc 不低于所需版本的基础镜像上重新构建（如 `debian:trixie-slim` 提供 2.41、`debian:bookworm-slim` 提供 2.36），或构建为静态二进制（Rust 用 `-C target-feature=+crt-static`，Go 用 `CGO_ENABLED=0`）。",
        (Lang::En, "rec.runtimecompat.muslhint") => "Either build a musl-target binary (`x86_64-unknown-linux-musl` for Rust, `CGO_ENABLED=0` for Go) or switch the base image to glibc (`debian:trixie-slim`, `ubuntu:24.04`, etc.).",
        (Lang::Zh, "rec.runtimecompat.muslhint") => "要么构建 musl 目标的二进制（Rust 用 `x86_64-unknown-linux-musl`，Go 用 `CGO_ENABLED=0`），要么把基础镜像换成 glibc 系（`debian:trixie-slim`、`ubuntu:24.04` 等）。",
        (Lang::En, "rec.runtimecompat.glibcmuslhint") => "If you intended this, no action needed. Otherwise, rebuild against glibc or switch the base image to Alpine to match.",
        (Lang::Zh, "rec.runtimecompat.glibcmuslhint") => "若属预期可忽略。否则请改为 glibc 目标重新构建，或把基础镜像换为 Alpine 以保持一致。",
        ...
        (Lang::En, "md.f.runtimelibc") => "Runtime libc",
        (Lang::Zh, "md.f.runtimelibc") => "运行时 libc",
        (Lang::En, "md.runtimelibc.ok") => "ok",
        (Lang::Zh, "md.runtimelibc.ok") => "兼容",
        (Lang::En, "md.runtimelibc.tooold") => "host glibc too old (will fail at startup)",
        (Lang::Zh, "md.runtimelibc.tooold") => "宿主 glibc 版本过低（启动会失败）",
        (Lang::En, "md.runtimelibc.glibconmusl") => "glibc binary on musl host (will not execute)",
        (Lang::Zh, "md.runtimelibc.glibconmusl") => "glibc 二进制运行在 musl 镜像（无法执行）",
        (Lang::En, "md.runtimelibc.muslonglibc") => "musl binary on glibc host (verify it loads)",
        (Lang::Zh, "md.runtimelibc.muslonglibc") => "musl 二进制运行在 glibc 镜像（请确认可加载）",
        ...
        (Lang::En, "tui.baseos") => "Base OS: ",
        (Lang::Zh, "tui.baseos") => "基础系统：",
        (Lang::En, "tui.runtimelibc") => "Runtime libc: ",
        (Lang::Zh, "tui.runtimelibc") => "运行时 libc：",
